...
Each task group contains a `service` stanza used to configure Consul
Connect, described below.

### API

The `api` task group contains a network stanza that configures the group to use
bridge mode. This is required for enabling tasks in the task group to use Connect:
...
The `api` task group also contains a `service` stanza which enables Connect. The
service stanza will cause two new entries in the Consul catalog: one for the `count-api`
service, and one for the implicit `count-api-sidecar-proxy` service.
...
the group to use bridge mode (as required for the task group to use Connect). It
also defines a static and forwarded port which will be used as the port on which
the Dashboard UI is made accessible.
