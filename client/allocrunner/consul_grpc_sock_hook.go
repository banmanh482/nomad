...
func (p *grpcSocketProxy) run(alloc *structs.Allocation) error {
	...
	go func() {
		proxy(p.ctx, p.logger, destAddr, true, listener)
		p.cancel()
		close(p.doneCh)
	}()
...
func proxy(ctx context.Context, logger hclog.Logger, destAddr string, bi bool, l net.Listener) {
	...
	for ctx.Err() == nil {
		...
		go func() {
			defer wg.Done()
			proxyConn(ctx, logger, destAddr, bi, conn)
		}()
	}
}
...
// to prevent leaks.
func proxyConn(ctx context.Context, logger hclog.Logger, destAddr string, bi bool, conn net.Conn) {
	...
	// consul -> socket, only when protocol is bi-directional gRPC
	if bi {
		wg.Add(1)
		go func() {
			defer wg.Done()
